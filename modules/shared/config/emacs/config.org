...
<<<<<<< HEAD
Why an ~org~ file? My Emacs config is large, and this enables me to improve the readability of its documentation greatly. I edit this file the same way you're reading it (nicely formatted) as I use Emacs and Emacs speaks ~org~. Sounds complicated, but it's really not; [[https://github.com/dlorent/nixos-config/blob/main/hosts/nixos/default.nix#L236][just a few lines]] in my Nix config. [[https://github.com/dlorent/nixos-config/blob/main/hosts/darwin/default.nix#L40][MacOS too.]]

This is the main configuration, but one more init file also exists, ~init.el~ , that bootstraps ~org-mode~ before this file is interpreted. That's defined [[https://github.com/dlorent/nixos-config/blob/main/modules/shared/files.nix#L56][here]].
...
=======
Why an org file? My Emacs config is large, and this enables me to greatly improve readability of its documentation. I edit this file the same way you're reading it (nicely formatted) as I use Emacs and Emacs speaks org. Sounds complicated, but it's really not; [[https://github.com/dlorent/nixos-config/blob/main/nixos/default.nix#L215][just a few lines]] in my Nix config. [[https://github.com/dlorent/nixos-config/blob/main/darwin/default.nix#L28][MacOS too.]]

This is the main configuration, but there also exists one more init file, ~init.el~ , that bootstraps ~org-mode~ before this file is interpreted. That's defined [[https://github.com/dlorent/nixos-config/blob/main/shared/files.nix#L5][here]].
...
<<<<<<< HEAD
  (setq user-full-name "dlorent "
    user-mail-address "dlorent@lorentsen.dev")
=======
  (setq user-full-name "Dennis"
    user-mail-address "dlorent@lorentsen.nu")
>>>>>>> b3202fa (Initial commit with changes)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Set path for darwin
  (when (system-is-mac)
    (setenv "PATH" (concat (getenv "PATH") ":/Users/dlorent/.nix-profile/bin:/usr/bin"))
    (setq exec-path (append '("/Users/dlorent/bin" "/profile/bin" "/Users/dlorent/.npm-packages/bin" "/Users/dlorent/.nix-profile/bin" "/nix/var/nix/profiles/default/bin" "/usr/local/bin" "/usr/bin") exec-path)))
#+END_SRC
...
<<<<<<< HEAD
    :straight (:host github :repo "dlorent/org-roam"
               :branch "master"
=======
